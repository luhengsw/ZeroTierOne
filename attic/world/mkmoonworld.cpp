...
int main(int argc, char** argv)
{
	...
	if (argc <= 1) {
		fprintf(stderr, "Usage: mkmoonworld [moon.json path] ..." ZT_EOL_S);
		return 1;
	}
	...
	for (auto& path : moonJsonFile) {
		...
		if (! data["roots"].is_array()) {
			fprintf(stderr, "moon.json has a wrong format!" ZT_EOL_S);
			return 2;
		}
		// Parse
		for (auto& root : data["roots"]) {
			...
			for (auto& addr : root["stableEndpoints"]) {
				...
				fprintf(stdout, "Add world at endpoints: %s" ZT_EOL_S, endpoint.c_str());
